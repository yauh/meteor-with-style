Template.linkList.helpers({
  links: function () {
    return LinksCollection.find({}, {sort: {votes: -1}});
  }
});
...
Template.link.events({
  'click button.btn-danger': function (evt) {
    // delete link
    evt.preventDefault();
    Meteor.call('RemoveLink', {linkId: this._id}, function (error, result) {
  ...
  'click button.btn-success': function (evt) {
    // upvote link
    evt.preventDefault();
    Meteor.call('AddVote', {linkId: this._id}, function (error, result) {
