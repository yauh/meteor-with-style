...
Meteor.methods({
  ...
  'AddVote': function (data) {
    ...
    if (LinksCollection.find({_id: data.linkId, voters: data.userId}).count() === 0) {
      return LinksCollection.update(data.linkId, {$push: {voters: data.userId}, $inc: {votes: 1}});
    }
    else {
  ...
  'RemoveLink': function (data) {
    ...
    if (this.userId !== LinksCollection.findOne({_id: data.linkId}).userId) {
